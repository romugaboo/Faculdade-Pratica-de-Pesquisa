...
calculate_punctuality <- function(data, year) {
  ...
  stats <- data_filtered %>%
    summarise(
      n = n(),
      delayed_flights = sum(delay_depart > 0, na.rm = TRUE),
      punctual_flights = sum(delay_depart == 0, na.rm = TRUE),
      punctuality_percentage = mean(delay_depart == 0, na.rm = TRUE),  # Punctuality percentage (flights on time)
      mean = punctuality_percentage,  # Mean punctuality percentage
      sd = sd(as.numeric(delay_depart == 0), na.rm = TRUE),  # Standard deviation of punctuality
      median = median(as.numeric(delay_depart == 0), na.rm = TRUE),
      trimmed = mean(as.numeric(delay_depart == 0), trim = 0.1, na.rm = TRUE),
      mad = mad(as.numeric(delay_depart == 0), na.rm = TRUE),
      min = min(as.numeric(delay_depart == 0), na.rm = TRUE),
      max = max(as.numeric(delay_depart == 0), na.rm = TRUE),
      range = max - min,
      skew = e1071::skewness(as.numeric(delay_depart == 0), na.rm = TRUE),
      kurtosis = e1071::kurtosis(as.numeric(delay_depart == 0), na.rm = TRUE),
      se = sd / sqrt(n),
