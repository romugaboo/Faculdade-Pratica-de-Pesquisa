...
calculate_punctuality_stats <- function(data, year) {
  data %>%
    # Remove cancelled flights, outliers, and flights with negative delay_depart values
    # Nao vai considerar os voos adiantados
    filter(status != "Cancelado", outlier_depart_delay == FALSE, delay_depart >= 0) %>%
    # Calculate metrics
    summarise(
...
write.csv(result_2023, paste0("rate_depart_", year, ".csv"), row.names = FALSE)
